...
function fetchCommits(baseTag, branch) {
  const format = "%H%x1f%P%x1f%an%x1f%cI%x1f%s%x1f%b%x00";
  const stdout = git(["log", `--format=${format}`, `${baseTag}..${branch}`]);

  return stdout.split("\x00").filter((r) => r.trim()).map((record) => {
...
function computeVersions(commits, commitToTag) {
  ...
  for (const hash of Object.keys(commits)) {
    ...
    if (v) {
      ...
      commits[hash].version = v.distance ? `${version} +${v.distance}` : version;
    }
  }
}
...
function main() {
  ...
  const branches = [
    "main", "stable", "latest",
    ...lines(git(["for-each-ref", "--format=%(refname:short)", "refs/heads/release/"])),
  ];
  ...
  const tags = {};
  const tagFormat = "%(refname:short) %(if)%(*objectname)%(then)%(*objectname)%(else)%(objectname)%(end)";
  for (const line of lines(git(["for-each-ref", `--format=${tagFormat}`, "refs/tags/v*"]))) {
    const [tag, hash] = line.split(" ");
    if(commits[hash]){
      tags[tag] = hash;
    }
  }
  tags[BASE_TAG] ??= git(["rev-parse", `${BASE_TAG}^{}`]).trim();
  ...
  console.log(`Written ${Object.keys(commits).length} commits to ${OUTPUT_PATH}`);
}
